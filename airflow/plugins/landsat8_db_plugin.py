...
class DownloadSceneList(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, download_url, download_dir,
    ...
    def execute(self, context):
        ...
        urllib.urlretrieve(self.download_url,
                           self.download_dir+"scene_list.gz")
        context['task_instance'].xcom_push(
            key='scene_list_gz_path',
            value=str(self.download_dir)+"scene_list.gz"
        )
        return True
...
class ExtractSceneList(BaseOperator):
    ...
    def execute(self, context):
        ...
        scene_list_gz_path = context['task_instance'].xcom_pull(
            'download_scene_list_gz_task', key='scene_list_gz_path')
        ...
        outF.close()
        context['task_instance'].xcom_push(
            key='scene_list_csv_path',
            value=str(self.download_dir)+"scene_list.csv"
        )
        return True
...
class UpdateSceneList(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, pg_dbname, pg_hostname, pg_port, pg_username,
    ...
    def execute(self, context):
        ...
        scene_list_csv_path = context['task_instance'].xcom_pull(
            'extract_scene_list_task', key='scene_list_csv_path')
        delete_first_line_cmd = (
            "tail -n +2 " + scene_list_csv_path + "> " +
            scene_list_csv_path + ".tmp && mv "+ scene_list_csv_path +
            ".tmp  " + scene_list_csv_path
        )
        delete_line_operator = BashOperator(
            task_id='Delete_first_line_OP',
            bash_command = delete_first_line_cmd
        )
