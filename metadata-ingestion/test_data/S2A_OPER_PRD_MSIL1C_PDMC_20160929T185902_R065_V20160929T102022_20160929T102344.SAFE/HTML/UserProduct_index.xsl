...
<xsl:template match="/">
  ...
  <h3>Selected bands</h3>
    ...
    <xsl:for-each select="//Product_Info/Product_Organisation/Granule_List">
    	Granule: <xsl:value-of select = "Granules/@granuleIdentifier"/> Image format: <xsl:value-of select = "./Granules/@imageFormat"/>
    	<xsl:for-each select="Granules/IMAGE_ID">
	...
	<h2>Product Image Characteristics</h2>
	  ...
	  <h1>Coordinate reference system</h1>
		...
		<table class="box-table-a">
		    <tr>
		      <th>Sensor quality flag</th>
		      <th>Geometric quality flag</th>
		      <th>General quality flag</th>
		      <th>Format correctness flag</th>
		    </tr>
		    <tr>
		      <td><xsl:value-of select="//Quality_Control_Checks/Quality_Inspections/SENSOR_QUALITY_FLAG"/></td>
		      <td><xsl:value-of select="//Quality_Control_Checks/Quality_Inspections/GEOMETRIC_QUALITY_FLAG"/></td>
		       <td><xsl:value-of select="//Quality_Control_Checks/Quality_Inspections/GENERAL_QUALITY_FLAG"/></td>
		      <td><xsl:value-of select="//Quality_Control_Checks/Quality_Inspections/FORMAT_CORRECTNESS_FLAG"/></td>
		    </tr>
  		</table>
